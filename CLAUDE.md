...
- **Screen layout like Claude Code** (layout A, picked by the owner from mockups on 2026-09-22; the middle of the screen
  ...
  and commands in `/help`. The mascot is `mascot.ARTS`, sizes from large to small (24×12 and 18×9 cells of (char, fg,
  bg) in 24-bit colour), drawn with **block elements** (`▀▄▌▐`, quadrants, eighths; U+2580–U+259F without `░▒▓`).
  `UI._mascot_art` takes the largest size that fits both the width (art + 34 columns) and the height (art +
  `HEADER_SPARE_ROWS`, so the whole start screen fits and a ~17-row VS Code panel gets 18×9); otherwise, and under
  `NO_COLOR`, only the three lines are printed; pipes get one plain line. The owner picked this from real renders on
  ...
  The data is generated by `agent-cli/tools/make_mascot.py` from `tools/mascot.png` (Pillow, dev only; `tools/` is not
  in the wheel, which only packs `peto_agent/**/*.py`): each cell picks the block and two colours closest to the image,
  transparent parts stay empty, dark outlines are thickened first. Change the image or sizes by rerunning that tool,
  never by editing `mascot.py`. The input (`line_editor.layout`) is drawn between two dim rules with no
