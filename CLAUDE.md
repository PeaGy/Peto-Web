...
- **Screen layout like Claude Code** (layout A, picked by the owner from mockups on 2026-09-22; the middle of the screen
  ...
  and commands in `/help`. The mascot is `mascot.ART`, 18×9 cells of (Braille char, fg, bg) in 24-bit colour, style
  "outline dots + filled background" picked from previews on 2026-09-22: dark outline pixels are the dots, the fill is
  the cell background, and edge cells skip the background (the terminal's own shows through) and draw the silhouette in
  the fill colour so it does not vanish on dark themes. It is generated by `agent-cli/tools/make_mascot.py` from
  `tools/mascot.png` (Pillow, dev only; `tools/` is not in the wheel, which only packs `peto_agent/**/*.py`), so change
  the image or size by rerunning that tool, never by editing `mascot.py`. Windows narrower than the art plus 34 columns
  get the three lines alone; `NO_COLOR` keeps the dots without colour codes; pipes get one plain line. The input (`line_editor.layout`) is drawn between two dim rules with no
