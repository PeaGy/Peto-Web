...
describe('Đọc thành tiếng', () => {
  ...
  beforeEach(() => {
    spoken.length = 0;
    cancels = 0;
    // jsdom không có Web Speech API; thiếu nó thì nút loa còn không hiện ra.
  ...
  const streamTwoSentences = () => {
    vi.mocked(api.sendMessage).mockImplementation(async (_payload, handlers) => {
      handlers.onMeta?.('C', 'low', row('chào'));
      handlers.onDelta?.('Chào cậu.');
      handlers.onDelta?.(' Hôm nay thế nào?');
      handlers.onDone?.();
    });
  };

  const send = () => {
    fireEvent.change(screen.getByPlaceholderText('Nhắn cho Peto…'), { target: { value: 'chào' } });
    fireEvent.click(screen.getByRole('button', { name: 'Gửi', exact: true }));
  };

  it('bật loa rồi gửi tin thì đọc từng câu một', async () => {
    streamTwoSentences();
    await openApp();
    fireEvent.click(screen.getByRole('button', { name: 'Đọc câu trả lời thành tiếng' }));
    // Câu xác nhận lúc bật vừa cho nghe thử giọng, vừa mở khóa âm thanh trên iOS.
    expect(spoken).toEqual(['Peto đọc câu trả lời nha.']);
    expect(JSON.parse(localStorage.getItem('peto-voice') ?? '{}').on).toBe(true);

    spoken.length = 0;
    send();
    await waitFor(() => expect(spoken).toEqual(['Chào cậu.', 'Hôm nay thế nào?']));
  });

  it('chưa bật thì im lặng', async () => {
    streamTwoSentences();
    await openApp();
    send();
    await waitFor(() => expect(api.sendMessage).toHaveBeenCalled());
  ...
  it('bấm Dừng thì cắt tiếng đang đọc', async () => {
    const pending = deferred<void>();
    vi.mocked(api.sendMessage).mockImplementation(async (_payload, handlers) => {
      handlers.onMeta?.('C', 'low', row('chào'));
      handlers.onDelta?.('Peto đang nói một câu dài. ');
  ...
  it('bật từ Cài đặt thì nút loa trong ô nhắn cũng sáng theo', async () => {
    await openApp();
    fireEvent.click(screen.getByRole('button', { name: /Cài đặt/ }));
    fireEvent.click(await screen.findByLabelText('Đọc câu trả lời của Peto'));
    expect(screen.getByRole('button', { name: 'Tắt đọc thành tiếng' }).getAttribute('aria-pressed')).toBe('true');
  });
});
