...
it('moves gently, stays within eye limits, and fades out on cursor activity', () => {
  ...
  expect(first.x).toBeGreaterThan(0); expect(first.x).toBeLessThan(0.04);
  let pose = first;
  for (let i = 0; i < 150; i++) pose = eyes.step(1 / 30, true);
  expect(Math.abs(pose.x)).toBeLessThanOrEqual(0.35);
  expect(Math.abs(pose.y)).toBeLessThanOrEqual(0.18);
